...
const DetailTransactionHistory: React.FC = () => {
  ...
  const { data, status }: any = useGetDetailBooking(
    { id: stringId },
    {
      onSuccess: (res: any) => {},
      onError: (err: any) => {
        ...
        {status === 'success' && (
          <div tw="flex flex-col">
            <CustomerInformationCard
              ...
              customerProfilePicture={detail.customer_image}
            />
            ...
            <div tw="grid grid-rows-6 grid-cols-11 gap-y-1 space-y-3">
              ...
              <div tw="row-start-1 col-start-6 col-span-3">
                <span tw="flex text-xl">
                  <p tw="font-bold whitespace-pre-wrap">: </p>{' '}
                  {moment.utc(detail.start_time).format('HH:mm')}-
                  {moment.utc(detail.end_time).format('HH:mm')} |{' '}
                  {moment.utc(detail.date).format('Do MMMM YYYY')}
                </span>
              </div>
              ...
              <div tw="row-start-2 col-start-6 col-span-3">
                <span tw="flex text-xl">
                  <p tw="font-bold whitespace-pre-wrap">: </p> {detail.capacity}{' '}
                  Tiket
                </span>
              </div>
              ...
              <div tw="row-start-3 col-start-6 col-span-3 ">
                <span tw="flex text-xl">
                  <p tw="font-bold whitespace-pre-wrap">: </p> Rp
                  {detail.total_price_ticket}
                </span>
              </div>
              ...
              {detail.items?.length === 0 ? (
                <p tw="col-start-6 col-span-3 text-xl justify-self-center text-gray-500">
                  Tidak Ada Pesanan Item
                </p>
              ) : (
                detail.items?.map((item_detail: any, index: string) => {
                  return (
                    <React.Fragment key={index}>
                      <p tw="col-start-6 col-span-2 text-xl">
                        {item_detail.name}
                      </p>
              ...
              <div tw="col-start-6 col-span-3 ">
                <span tw="flex text-2xl font-bold ">
                  <p tw="whitespace-pre-wrap">: </p> Rp{detail.total_price}
                </span>
              </div>
            </div>
          </div>
